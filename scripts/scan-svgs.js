...
// 从文件名提取标题
function extractTitleFromFilename(filename, mappings) {
...
// 扫描单个分类目录
async function scanCategory(categoryDir, mappings) {
    console.log(`DEBUG: 开始扫描 ${basename(categoryDir)}`);
    const categorySlug = basename(categoryDir);
    const metaFile = join(categoryDir, '_meta.yaml');
    // 读取现有元数据
    let existingMeta = { items: [] };
    if (existsSync(metaFile)) {
        const content = await readFile(metaFile, 'utf-8');
        existingMeta = YAML.parse(content) || existingMeta;
    }
    console.log(`DEBUG: 读取到 ${existingMeta.items.length} 个现有条目`);
    // 获取所有 SVG 文件
    const files = await readdir(categoryDir);
    const svgFiles = files.filter(f => f.endsWith('.svg') && !f.startsWith('.'));
    console.log(`DEBUG: 找到 ${svgFiles.length} 个 SVG 文件`);
    // 已记录的文件
    const recordedFiles = new Set(existingMeta.items.map((item) => {
    ...
    console.log(`DEBUG: 已记录 ${recordedFiles.size} 个文件`);
    // 查找新文件
    const newFiles = svgFiles.filter(f => !recordedFiles.has(f));
    console.log(`DEBUG: 发现 ${newFiles.length} 个新文件`);
    // 查找已删除的文件
    const deletedFiles = [];
    ...
    console.log(`DEBUG: 发现 ${deletedFiles.length} 个已删除文件`);
    // 生成新条目
    const newItems = newFiles
        ...
        // 查找对应的 wordmark
        const wordmarkFile = svgFiles.find(f => f.startsWith(baseName + '_wordmark'));
        const item = {
            title,
            file,
            url: 'TODO'
    ...
    console.log(`DEBUG: 生成 ${newItems.length} 个新条目`);
    console.log(`DEBUG: 返回 { newItems: ${newItems.length}, deletedItems: ${deletedItems.length} }`);
    return { newItems, deletedItems };
}
// 更新元数据文件
async function updateMetaFile(metaFile, newItems) {
    // 读取现有元数据
    let existingMeta = { items: [] };
    if (existsSync(metaFile)) {
        const content = await readFile(metaFile, 'utf-8');
